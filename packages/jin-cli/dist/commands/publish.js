...
async function publish(options = {}, cwd = process.cwd()) {
    ...
    const relPath = path_1.default.relative(cwd, jinJsonPath) || 'jin.json';
    ...
    if (!options.skipDeploy) {
        ...
        if (ans === 'y' || ans === 'yes') {
            ...
            try {
                (0, child_process_1.execSync)(`git add ${relPath}`, { stdio: 'pipe' });
                console.log('  ✓ Staged\n');
            }
            catch (err) {
            ...
            try {
                (0, child_process_1.execSync)('git commit -m "feat: add AIP intent map (jin v0.1)"', { stdio: 'pipe' });
